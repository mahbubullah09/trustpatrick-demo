      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-3">
        {cities.map((city) => (
          <Link
            ...
            className="card px-4 py-3 text-sm font-medium text-brand-navy hover:text-brand-blue hover:border-brand-blue transition-colors"
          >
            {city.city_name}
          </Link>
        ))}
          ...
          {services.map((s) =>
            cities.map((c) => (
              <Link
                ...
                className="text-sm bg-brand-light text-brand-blue border border-blue-100 px-3 py-1.5 rounded-full hover:bg-brand-blue hover:text-white transition-colors"
              >
                {s.shortName} – {c.city_name}
              </Link>
            ))
          )}
        </div>

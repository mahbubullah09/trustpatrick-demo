...
import { getAllRegions, getCountiesInRegion, slugify } from '@/data/locations';

export const metadata: Metadata = {
  title: 'Counties — Sitemap | TrustPatrick',
};

export default function CountiesSitemap() {
  const allRegions = getAllRegions().sort((a, b) => a.region_name.localeCompare(b.region_name));

  const grouped = allRegions.map((r) => ({
    region: r,
    counties: getCountiesInRegion(r.region_code).sort((a, b) => a.city_name.localeCompare(b.city_name)),
  })).filter((g) => g.counties.length > 0);

  const totalCounties = grouped.reduce((sum, g) => sum + g.counties.length, 0);
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
      <nav className="text-sm text-brand-gray mb-4 flex items-center gap-1 flex-wrap">
        ...
        <span>/</span>
        <span className="text-brand-navy font-medium">Counties</span>
      </nav>
      <h1 className="section-title mb-2">Counties</h1>
      <p className="text-brand-gray mb-10">{totalCounties.toLocaleString()} counties across {grouped.length} states</p>

      <div className="space-y-10">
        {grouped.map(({ region, counties }) => (
